...
__xdata uint8_t HIDKey[9] = {0x1,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0};
...
uint8_t Keyboard_press(uint8_t k) {
	...
	if (k >= 136) {			// it's a non-printing key (not a modifier)
		...
		if (k & 0x80) {						// it's a capital letter or other character reached with shift
			HIDKey[1] |= 0x02;	// the left shift modifier
			k &= 0x7F;
		}
	}
	...
	if (HIDKey[3] != k && HIDKey[4] != k && 
		HIDKey[5] != k && HIDKey[6] != k &&
...
void Keyboard_releaseAll(void){
    for (uint8_t i=1;i<sizeof(HIDKey);i++){                                  //load data for upload
        HIDKey[i] = 0;
    }
	USB_EP1_send();
}
...
uint8_t Keyboard_getLEDStatus(){
    return Ep1Buffer[1];    //The only info we gets
}
